...
author: "par NOM1 Prénom1 et NOM2 Prénom2"
date: "xx décembre 2017" 
output:
  html_document:
    fig_caption: yes
---
...
Pour les commentaires, on peut utiliser `r fig$ref("cos_wav (label)")` pour faire la référence.

([more details](https://github.com/ajlyons/autonumfigs))

Si on veut une figure sans caption :

```{r, fig.cap=fig$cap(), echo = FALSE}
plot(rnorm(100))
```

**Attention !** Il faut evider le cas où un code bloque
crée 2 figures. On pourrais consider à faire une
matrice de figures.


```{r, fig.width = 10, fig.height = 8, fig.cap=fig$cap("lm", "diagonistic de modèle linéaire"), echo = FALSE}
